...
// 삽입연산들
void d_linked_list::push_front(int n){ // 리스트의 맨 앞에 노드를 추가합니다.
...
void d_linked_list::push_back(int n){ // 리스트의 끝에 노드를 추가합니다.
...
void d_linked_list::insert_node_at(int idx, int n){ // index에 있는 값을 지웁니다.
    if(size < idx || 0 > idx){
        ...
        new_node->data = n;
        new_node->pre = nullptr;  // 새 node를 일단 초기화해줍니다.
...
// 삭제연산
int d_linked_list::pop_front(){ // 리스트의 맨 앞 노드를 지웁니다.
    if(head == nullptr){
...
int d_linked_list::pop_node_at(int idx){ // index에 있는 값을 지웁니다.
    int value = -1;
...
void d_linked_list::remove_node(int n){ // 값을 찾아서 한 개를 지웁니다.
    node *ptr = head;
    node *tmp = ptr;
    while(ptr != nullptr){
        if(ptr->data == n){
...
void d_linked_list::delete_list(){
    node *ptr = head;
    while(ptr != nullptr){
        head = ptr->next;
        delete ptr;
...
// 대입
void d_linked_list::replace_value_at(int idx, int n){
    node *ptr = head;
...
// 검색 및 표시
int d_linked_list::Size(){
    return size;
}

void d_linked_list::display_contents(){
    node *ptr = head;
    while(ptr != nullptr){
        std::cout << ptr->data << "->";
        ptr = ptr->next;
...
int d_linked_list::search_value(int n){
    node *ptr = head;
    int idx = 0;
    bool isFind = false;
    while(ptr != nullptr){
...
int main(){
    d_linked_list dl_list;
    dl_list.push_back(1);
    dl_list.push_front(2);
    dl_list.push_back(3);
    dl_list.push_front(4);
    dl_list.display_contents();
    dl_list.pop_node_at(3);
    dl_list.display_contents();

    std::cout << dl_list.head->next->next->pre->data << std::endl;
    std::cout << dl_list.tail->pre->data << std::endl;
